...
#### Класс ContactForm
Форма ввода контактных данных. Для кнопки "Оплатить" в классе устанавливается обработчик клика, в результате работы которого должен отправляться запрос на сервес. При успешном запросе должен отрисовываться контент класса `Success`
